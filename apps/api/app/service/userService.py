...
from app.db.schema.user import UserOutput , UserInLogin , UserWithToken , UserInCreate
...
class UserService:
    ...
    def signup(self, user_details: UserInCreate) -> UserOutput:
        if self.__userRepository.user_exist_by_email(email=user_details.email):
            raise HTTPException(status_code=400 , detail="Please Login Email Exists")
    ...
    def login(self, login_details : UserInLogin) -> UserWithToken:
        ...
        if HashHelper.verify_password(plain_password=login_details.password , hashed_password=user.password):
            ...
            raise HTTPException(status_code=500 , detail="Unable to process request :0 ")
        
        raise HTTPException(status_code=400 , detail="Please Check and Verify Your Credentials")
